...
class mySeq2SeqModel(tf.keras.Model):
    def __init__(self):
        ...
        self.encoder = tf.keras.layers.RNN(self.encoder_cell, return_sequences=True, return_state=True)
    ...
    def call(self, enc_ids, dec_ids):
        # Encoder处理
        enc_emb = self.embed_layer(enc_ids)  # [batch, enc_len, emb_dim]
        enc_out, enc_state = self.encoder(enc_emb)  # enc_out: [batch, enc_len, hidden]
        
        # Decoder处理
        dec_emb = self.embed_layer(dec_ids)  # [batch, dec_len, emb_dim]
        dec_seq_len = tf.shape(dec_ids)[1]
        ...
        for t in range(dec_seq_len):
            ...
            # 计算注意力权重
            # 双线性注意力: score = enc_out * W * state^T
        ...
        # 堆叠所有时间步的输出
        logits = tf.stack(outputs, axis=1)  # [batch, dec_len, vocab_size]
        return logits
...
def train(model, optimizer, seqlen=10, steps=2000):
...
def sequence_reversal(model, seq_len=10):
    ...
    return decode(state, seq_len, enc_out), batched_examples
...
if __name__ == "__main__":
    ...
    # 训练模型
    print("开始训练带注意力机制的Seq2Seq模型...")
    train(model, optimizer, seqlen=20)
    
    # 测试模型
    print("\n测试结果:")
    reversed_seqs, original_seqs = sequence_reversal(model, seq_len=20)
